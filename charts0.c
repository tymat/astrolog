/*
** Astrolog (Version 5.41G) File: charts0.c
**
...
byte *GetSzGenitive(szGen, szInst)
byte *szGen, *szInst;
{
  byte *pch, ch1, ch2;
  int cch;
...
int i;
{
  byte sz1[cchSzDef], sz2[cchSzDef], *pchSpace, *szInst;

  sprintf(szGen, "%s", szCnstlName[i]);
  for (pchSpace = szGen; *pchSpace && *pchSpace != ' '; pchSpace++)
    ;
  szInst = (byte *)szCnstlGenitive[i];
  if (*pchSpace == chNull) {
...
{
  int i, j = eWat;
  byte szGen[cchSzDef], sz[cchSzDef], chLast = chNull;
