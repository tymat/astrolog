/*
** Astrolog (Version 5.41G) File: charts1.c
**
...
void PrintHeader()
{
  ...
  if (us.nRel == rcDual) {
    ...
    sprintf(sz, "%s %s synastry chart, houses ", szAppName, szVersionCore); PrintSz(sz);
  } else {
  ...
  if (fNoTimeOrSpace)
    ...
    if (us.nRel == rcMidpoint)
      PrintSz("(Time end space midpoint)\n");
    else {
      sprintf(sz, "for %s%s", ciMain.nam, "\n"); PrintSz(sz);
    }
...
void ChartListing()
{
  ...
  for (i = 1-us.fSeconds, j = 0; i <= oNorm; i++, j++) {
    if (us.fSeconds) {
      ...
      if (i > oCore+1 && ((i < cuspHi) || FIgnore(i)))
        continue;
      while (i <= cuspHi && j <= cuspHi && FIgnore(j))
        j++;
    }

    if (i >= cuspLo && i <= cuspHi-2 && j > cuspHi-2) /* Don't print blank */
       continue;                                      /* lines.      VA.   */

    if (i <= cuspHi-2  && j > cuspHi-2)
      PrintTab(' ', 52);
    else {
      if (i > cuspHi-2)
        j = i;
      ...
      sprintf(sz, "%c] ", Dignify(j, l)); PrintSz(FCusp(j) ? " " : sz);
      if ((j != oMoo || us.fPlacalc) &&
        (FObject(j) || ((j == oNod || j == oLil) && us.fPlacalc))) {
        PrintCh((byte)(ret[i] < 0.0 ? '-' : '+'));
...
void ChartGrid()
{
  ...
  for (y1 = 0, y = 0; y <= cObj; y++) if (FProper(y))
    for (r = 1; r <= 4; r++) {
      for (x1 = 0, x = 0; x <= cObj; x++) if (FProper(x)) {
        if (y1 > 0 && x1 > 0 && y+r > 1)
...
int i1, i2, i3, i4;
{
  ...
  case acMR: asp = aSex; break;
  case acK : asp = aTri; break;
  default: ;
  }
  AnsiColor(kAspA[asp]);
  ...
  sprintf(sz, " %s ", ac == acS || ac == acGT || ac == acGC ||
     ac == acMR || ac == acK ? "with" : "from"); PrintSz(sz);
  AnsiColor(kObjA[i1]);
      ...
      for (j = 0; j < i; j++) if (!FIgnore(j))
        if (k = grid->n[j][i]) {
          ...
          p = (int)(rAspInf[k]*(sqrt)(ip*jp)*
            (1.0-RAbs((real)(grid->v[j][i]))/60.0/GetOrb(i, j, k))*500.0);
          if (us.fParallel)
            p = (int)(p * PowerPar);
...
ChartHorizon()
{
  byte sz[cchSzDef], szFormat[cchSzDef];
  ...
  for (k = 1; k <= tot; k++) {
    ...
    if (ignore[i]   /*  || !FThing(i) */ )
      continue;
    AnsiColor(kObjA[i]);
    sprintf(sz, "%-7.7s: ", szObjName[i]); PrintSz(sz);
    PrintAltitude(alt[i]);

    /* Determine directional vector based on azimuth. */

    if (fNESW && !fPrime) {
      sprintf(sz, " %s", SzDegree(Mod(rDegQuad-azi[i]))); PrintSz(sz);
    } else {
...
void ChartSector()
{
  ...
  for (i = 0; i <= cObj; i++) {
    ...
    sprintf(sz, " %c", (byte)(pls ? '+' : '-')); PrintSz(sz);
...
void ChartAstroGraph()
{
  ...
  real planet1[objMax], planet2[objMax], mc[objMax], ic[objMax],
    ...
    lo, longm, w, x, y, z, ad, oa, am, od, dm;
