/*
** Astrolog (Version 5.41F) File: xcharts2.c
**
...
void XChartGridRelation()
{
  ...
  for (y = 0, j = oEar-2; y <= gs.nGridCell; y++) {
    do {
      j++;
    } while ((ignore[j] || j == us.objCenter) && j != -1 && j <= cObj);
    DrawColor(gi.kiGray);
...
/*    DrawEdge(y*unit, y*unit, (y+1)*unit, (y+1)*unit); */
    if (j <= cObj) for (x = 0, i = oEar-2; x <= gs.nGridCell; x++) {
      do {
        i++;
      } while ((ignoreT[i] || i == us.objCenter) && i != -1 && i <= cObj);
